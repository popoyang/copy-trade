        ...
        try {
            ...
            for (PositionRisk positionRisk : positionRisks) {
                ...
                if (Constants.BOTH.equalsIgnoreCase(positionSide)) {
                    return amt.compareTo(BigDecimal.ZERO) != 0 ? positionRisk : null;
                }

                if (amt.compareTo(BigDecimal.ZERO) > 0 && Constants.LONG.equalsIgnoreCase(positionSide)) {
                    return positionRisk;
                }

                if (amt.compareTo(BigDecimal.ZERO) < 0 && Constants.SHORT.equalsIgnoreCase(positionSide)) {
                    return positionRisk;
                }
            }
